...
START_TEST(is_less_or_equal_test_5) {
  ...
  ck_assert_int_eq(is_less_or_equal(dec2, dec1), 0);
}
END_TEST
...
START_TEST(is_less_or_equal_test_8) {
  ...
  from_float_to_decimal(num1, &dec1);
  from_float_to_decimal(num2, &dec2);
  int res1 = is_less_or_equal(dec1, dec2);
  int res2 = is_less_or_equal(dec2, dec1);
...
START_TEST(is_less_or_equal_test_9) {
  ...
  int result = is_less_or_equal(dec1, dec2);
...
Suite *test_is_less_or_equal(void) {
  ...
  tcase_add_test(tc, is_less_or_equal_test_13);
  tcase_add_test(tc, is_less_or_equal_test_14);
